...
If frmInventory.ObjLst.ListIndex > -1 Then
    SendData "DRP" & frmInventory.ObjLst.ListIndex + 1 & "," & frmInventory.DrpAmountTxt.Text
End If
...
If frmInventory.DrpAmountTxt.Text < 1 Then
    frmInventory.DrpAmountTxt.Text = MAX_INVENTORY_OBJS
End If

If frmInventory.DrpAmountTxt.Text > MAX_INVENTORY_OBJS Then
    frmInventory.DrpAmountTxt.Text = 1
End If
...
Private Sub SendTxt_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    stxtbuffer = SendTxt.Text
        'Turn off music
        If LCase(stxtbuffer) = "/midioff" Then
        ...
        'Whisper
        ElseIf LCase(Left$(stxtbuffer, 4)) = "/msg" Then
            SendData ("/msg" & Right$(stxtbuffer, Len(stxtbuffer) - 4))
    
        'Version display
        ElseIf LCase(Left$(stxtbuffer, 8)) = "/version" Then
            AddtoRichTextBox frmMain.RecTxt, "EraOnline Client, Version " & MyVersion, 0, 255, 0, 0, 0
            frmMain.RecTxt.SelStart = Len(frmMain.RecTxt.Text)
        
        'Show up tips
        ElseIf LCase(Left$(stxtbuffer, 8)) = "/tips" Then
            frmTips.Show
            
        'Emote
        ElseIf LCase(Left$(stxtbuffer, 6)) = "/me" Then
            SendData ("/me" & Right$(stxtbuffer, Len(stxtbuffer) - 6))
...
On Error Resume Next
If BackToMainMenu = True Then
frmMain.Hide
frmMenu.Show
End If
Dim i As Long
    
If BackToLogin = False Then
    prgRun = False
    Connected = False
Else
    ...
    Unload frmBook
    Unload frmDeposit
    Unload frmDonateGold
    Unload frmDropGold
    Unload frmGameOptions
    Unload frmGMCall
    Unload frmGMCommandList
    Unload frmInventory
    Unload frmMessage
    Unload frmSetup
    Unload frmTips
...
'Send use command
If frmInventory.ObjLst.ListIndex > -1 Then
    SendData "USE" & frmInventory.ObjLst.ListIndex + 1
End If
