...
Function TimeHour() As Integer
TimeHour = Val(Hour(Time))
End Function

Function TimeMinute() As Integer
TimeMinute = Val(Minute(Time))
End Function
...
Function CheckIPBan(userindex As Integer)
On Error Resume Next
...
'get number of IPs
NumIPs = Val(GetVar(IniPath & "ipban.txt", "INIT", "NumBANs"))

For IPs = 1 To NumIPs

'Load IP into memory and check if match, if not resume
IP = GetVar(IniPath & "ipban.txt", "IPS", "IP" & IPs)

If IP = UserList(userindex).IP Then
Call SendData(ToIndex, userindex, 0, "!!You are currently banned from Era Online. You may not enter with this or anyother old/new character. If you wish to complain, contact support@eraonline.net")
CloseSocket (userindex)
Exit Function
End If
