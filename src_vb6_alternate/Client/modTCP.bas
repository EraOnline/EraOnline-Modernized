...
If Left$(rData, 2) = "!!" Then
    ...
    MsgBox rData, vbApplicationModal + vbCritical + vbOKOnly, "EraOnline"
    Exit Sub
End If
...
If Left$(rData, 1) = "!" Then
    rData = Right$(rData, Len(rData) - 1)
    MsgBox rData, vbInformation + vbOKOnly, "EraOnline"
    Exit Sub
End If
...
If Left$(rData, 3) = "SST" Then
    ...
    frmInventory.RepRank = ReadField(47, rData, 44)
   ...
   frmMain.STAShp.Height = (((UserMinSTA / 100) / (UserMaxSTA / 100)) * 150)
    ...
    frmInventory.drink = UserDrink
    frmInventory.food = UserFood
...
frmInventory.gold.Caption = UserGLD
frmInventory.food.Caption = UserFood
frmInventory.drink.Caption = UserDrink
frmInventory.strenght.Caption = UserMaxHP
frmInventory.mana.Caption = UserMaxMAN
frmInventory.fatigue.Caption = UserMaxSTA
frmInventory.navn.Caption = UserName
frmInventory.Class.Caption = UserClass
...
If Left$(rData, 3) = "SIS" Then
    ...
    UserInventory(Slot).value = Val(ReadField(7, rData, 44))
    ...
    frmInventory.ObjLst.List(Slot - 1) = TempStr
    frmTrade.yourinv.List(Slot - 1) = TempStr
...
If Left$(rData, 3) = "NIS" Then
    ...
    frmTrade.shopinv.List(Slot - 1) = TempStr
    frmTrade.Show
...
Call DrawGrhtoHdc(frmInventory.body.hDC, CurrentGrh, 0, 0, 0, 0, SRCCOPY)
frmInventory.body.Picture = frmInventory.body.Image
Exit Sub
End If
...
CurrentGrh.FrameCounter = 1
CurrentGrh.SpeedCounter = GrhData(CurrentGrh.GrhIndex).Speed
Call DrawGrhtoHdc(frmInventory.weapon.hDC, CurrentGrh, 0, 0, 0, 0, SRCCOPY)
frmInventory.weapon.Picture = frmInventory.weapon.Image
Exit Sub
End If
...
CurrentGrh.FrameCounter = 1
CurrentGrh.SpeedCounter = GrhData(CurrentGrh.GrhIndex).Speed
Call DrawGrhtoHdc(frmInventory.shield.hDC, CurrentGrh, 0, 0, 0, 0, SRCCOPY)
frmInventory.shield.Picture = frmInventory.shield.Image
Exit Sub
End If
...
CurrentGrh.FrameCounter = 1
CurrentGrh.SpeedCounter = GrhData(CurrentGrh.GrhIndex).Speed
Call DrawGrhtoHdc(frmInventory.head.hDC, CurrentGrh, 0, 0, 0, 0, SRCCOPY)
frmInventory.head.Picture = frmInventory.head.Image
Exit Sub
End If
...
'Caracter Erased
If Left$(rData, 3) = "ERA" Then
    BackToLogin = True
    Exit Sub
End If

'Idle
If Left$(rData, 3) = "IDL" Then
    BackToLogin = True
    Exit Sub
End If
...
frmBook.news.Text = StrConv(InputB(LOF(1), 1), vbUnicode)
Close #1
Exit Sub
End If
