...
Sub MOTD(MOTD As String)
On Error Resume Next
Call WriteVar(IniPath & "Server.ini", "INIT", "MOTD", MOTD & FONTTYPE_FIGHT)
End Sub
...
Select Case map
Case 22, 49, 79, 155, 169, 170, 188, 189
    GoTo GoOn
End Select
...
On Error Resume Next
'**********************************************************
'CLEAR THE MAP FOR JUNK SUCH AS TOMBSTONES/CORPSES/LOCKS...
'**********************************************************
...
Sub TileWipe(userindex As Integer)
...
If UserList(userindex).Pos.x > XMaxMapSize - 3 Or UserList(userindex).Pos.x < XMinMapSize + 3 Then
    Call SendData(ToIndex, userindex, 0, "@Please go further from the mapborder." & FONTTYPE_INFO)
End If
If UserList(userindex).Pos.y > YMaxMapSize - 3 Or UserList(userindex).Pos.y < YMinMapSize + 3 Then
    Call SendData(ToIndex, userindex, 0, "@Please go further from the mapborder." & FONTTYPE_INFO)
End If
    
    For y = UserList(userindex).Pos.y - 3 To UserList(userindex).Pos.y + 3
    For x = UserList(userindex).Pos.x - 3 To UserList(userindex).Pos.x + 3
    
    MapData(map, x, y).Locked = 0
...
If UserList(userindex).Stats.EXP >= UserList(userindex).Stats.ELU Then
    ...
    UserList(userindex).Stats.EXP = 0
    UserList(userindex).Stats.ELU = Int(UserList(userindex).Stats.ELU * 1.2)
    ...
    UserList(userindex).Stats.DEF = UserList(userindex).Stats.DEF + 1
    UserList(userindex).Stats.DEF = UserList(userindex).Stats.DEF + 1
    
    SendData ToIndex, userindex, 0, "@Level up! You gained 5 training points, and your attributes has gone up !" & FONTTYPE_INFO
...
If NPCList(Npcindex).Tamed <> 0 Then
'Caused a strange bug...
'    'make it 50% stronger
'    NPCList(Npcindex).Stats.DEF = Int(NPCList(Npcindex).Stats.DEF * 2)
'    NPCList(Npcindex).Stats.MaxHIT = Int(NPCList(Npcindex).Stats.MaxHIT * 2)
'    NPCList(Npcindex).Stats.MaxHP = Int(NPCList(Npcindex).Stats.MaxHP * 2)
'    NPCList(Npcindex).Stats.MinHIT = Int(NPCList(Npcindex).Stats.MinHIT * 2)
'    NPCList(Npcindex).Stats.MinHP = Int(NPCList(Npcindex).Stats.MinHP * 2)
'    NPCList(Npcindex).GiveEXP = Int(NPCList(Npcindex).GiveEXP * 2)
'    NPCList(Npcindex).GiveGLD = Int(NPCList(Npcindex).GiveGLD * 2)
    NPCList(Npcindex).Tamed = 0
    UserList(NPCList(Npcindex).Owner).Stats.OwnAnimal = 0
    UserList(NPCList(Npcindex).Owner).Stats.AnimalIndex = 0
...
If LOOT > 1 Then
Call SendData(ToIndex, userindex, 0, "@Look at the corpse, you discover there is no loot on it." & FONTTYPE_INFO)
End If
...
If UserList(userindex).Flags.Meditate = 1 Then
    Call EraseObj(ToMap, 0, UserList(userindex).Pos.map, MapData(UserList(userindex).Pos.map, UserList(userindex).Pos.x, UserList(userindex).Pos.y).ObjInfo.Amount, UserList(userindex).Pos.map, UserList(userindex).Pos.x, UserList(userindex).Pos.y)
    UserList(userindex).Flags.Meditate = 0
    Call SendData(ToIndex, userindex, 0, "ME2")
    Call SendData(ToIndex, userindex, 0, "@Upon your death you snap out of your meditation trance." & FONTTYPE_INFO)
End If
...
SendData ToIndex, userindex, 0, "@As you stumble, you lose your food and water." & FONTTYPE_FIGHT
...
LoseExp = Int(UserList(userindex).Stats.EXP / 6)
LoseGold = Int(UserList(userindex).Stats.GLD / 5)

UserList(userindex).Stats.MinHP = UserList(userindex).Stats.MaxHP
SendData ToIndex, userindex, 0, "@Alas, you have been slaughtered to the death by a foe." & FONTTYPE_FIGHT
...
SendData ToIndex, userindex, 0, "@As your soul drifts away from your body, you see looters take " & LoseGold & " gold." & FONTTYPE_FIGHT
End If

'Loose 100 exp if over level 5 and have more than 99 exp
If UserList(userindex).Stats.ELV > 5 Then
UserList(userindex).Stats.EXP = UserList(userindex).Stats.EXP - LoseExp
SendData ToIndex, userindex, 0, "@Upon your death, " & LoseExp & " experience has been lost, as your mind recovers from the horrific experience." & FONTTYPE_FIGHT
End If
...
Call SendData(ToIndex, userindex, 0, "@As your body collapses, you drop your " & ObjData(UserList(userindex).Object(Item).ObjIndex).Name & "." & FONTTYPE_FIGHT)
...
Call SendData(ToIndex, userindex, 0, "@You try to use the equipment, but cannot manage to do so. This equipment requires " & obj.Level & " health to be equiped." & FONTTYPE_INFO)
Exit Sub
End If

'Check to see if class allows it
If UserList(userindex).class = obj.ClassForbid1 Then
Call SendData(ToIndex, userindex, 0, "@This item is not available for your class to equip!" & FONTTYPE_INFO)
Exit Sub
End If
...
Call SendData(ToIndex, userindex, 0, "@ Un-morph yourself to perform this action!" & FONTTYPE_INFO)
Exit Sub
End If
...
Select Case obj.ObjType
    ...
    Call SendData(ToIndex, userindex, 0, "|Welcome to the magical kingdom of Menath! Feel free to explore and interact with other adventurers as you go. You have been given a small amount of gold, water and food, as well as some clothing and a weapon to start off with. Good luck, adventurer!")
   ...
   SendData ToIndex, userindex, 0, "PLW" & SOUND_SWORDSWING
       ...
       SendData ToIndex, userindex, 0, "PLW" & SOUND_FOLDCLOTHING
        ...
        UserList(userindex).Throw.DrawingType = OBJTYPE_CARPENTRYDRAWING
        SendData ToIndex, userindex, 0, "PLW" & SOUND_PAPER
        Call SendData(ToIndex, userindex, 0, "@You view the drawing, all you need are some planks of wood to make the item you have selected." & FONTTYPE_INFO)
           
           
        Case OBJTYPE_PLANKS
        If UserList(userindex).Throw.DrawingType <> OBJTYPE_CARPENTRYDRAWING Then Exit Sub
        ...
        UserList(userindex).Throw.skill = obj.skill
        UserList(userindex).Throw.DrawingType = OBJTYPE_TAILORDRAWING
        SendData ToIndex, userindex, 0, "PLW" & SOUND_PAPER
        ...
        If UserList(userindex).Throw.DrawingType <> OBJTYPE_TAILORDRAWING Then Exit Sub
        ...
        UserList(userindex).Throw.skill = obj.skill
        UserList(userindex).Throw.DrawingType = OBJTYPE_BLACKSMITHINGDRAWING
        SendData ToIndex, userindex, 0, "PLW" & SOUND_PAPER
        ...
        Case OBJTYPE_STEEL
        If UserList(userindex).Throw.DrawingType <> OBJTYPE_BLACKSMITHINGDRAWING Then Exit Sub
...
If NPCList(Npcindex).Hostile Then
    ...
    For HeadingLoop = NORTH To WEST
        ...
        If InMapBounds(nPos.map, nPos.x, nPos.y) Then
            If MapData(nPos.map, nPos.x, nPos.y).userindex > 0 And UserList(MapData(nPos.map, nPos.x, nPos.y).userindex).Flags.status <> 1 Then
                'Face NPC to target
...
'Guard attack player if criminal
...
'ChangeNPCChar ToMap, 0, nPos.map, Npcindex, NPCList(Npcindex).Char.Body, NPCList(Npcindex).Char.Head, HeadingLoop2, NPCList(Npcindex).Char.WeaponAnim, NPCList(Npcindex).Char.ShieldAnim
                ChangeNPCChar ToNPCArea, Npcindex, nPos.map, Npcindex, NPCList(Npcindex).Char.Body, NPCList(Npcindex).Char.Head, HeadingLoop2, NPCList(Npcindex).Char.WeaponAnim, NPCList(Npcindex).Char.ShieldAnim
                'Attack
        ...
        If InMapBounds(nPos.map, nPos.x, nPos.y) Then
            If MapData(nPos.map, nPos.x, nPos.y).userindex > 0 And UserList(MapData(nPos.map, nPos.x, nPos.y).userindex).Flags.status <> 1 Then
                ...
                'Attack if player is human or dark elf
                 If UserList(MapData(nPos.map, nPos.x, nPos.y).userindex).Race = "Human" Or UserList(MapData(nPos.map, nPos.x, nPos.y).userindex).Race = "Wood Elf" Then
    ...
    Case 3
        For y = NPCList(Npcindex).Pos.y - 10 To NPCList(Npcindex).Pos.y + 10    'Makes a loop that looks at
            For x = NPCList(Npcindex).Pos.x - 10 To NPCList(Npcindex).Pos.x + 10   '10 tiles in every direction
                ...
                'look for a user
                    If MapData(NPCList(Npcindex).Pos.map, x, y).userindex > 0 And useai = 2 And UserList(MapData(nPos.map, x, y).userindex).Flags.status <> 1 Then
                        'Move towards user
...
Case 4
        ...
        For x = NPCList(Npcindex).Pos.x - 10 To NPCList(Npcindex).Pos.x + 10
                ...
                If x > MinXBorder And x < MaxXBorder And y > MinYBorder And y < MaxYBorder Then
                    ...
                    'look for a user
                    If MapData(NPCList(Npcindex).Pos.map, x, y).userindex > 0 And UserList(MapData(nPos.map, x, y).userindex).Flags.status <> 1 Then
        ...
        For x = NPCList(Npcindex).Pos.x - 10 To NPCList(Npcindex).Pos.x + 10
                ...
                If x > MinXBorder And x < MaxXBorder And y > MinYBorder And y < MaxYBorder Then
                    ...
                    If MapData(NPCList(Npcindex).Pos.map, x, y).userindex > 0 And UserList(MapData(nPos.map, x, y).userindex).Flags.status <> 1 Then
                        ...
                        If UserList(MapData(NPCList(Npcindex).Pos.map, x, y).userindex).Race = "Wood Elf" Or UserList(MapData(NPCList(Npcindex).Pos.map, x, y).userindex).Race = "Human" Then
                            tHeading = FindDirection(NPCList(Npcindex).Pos, UserList(MapData(NPCList(Npcindex).Pos.map, x, y).userindex).Pos)
                            MoveNPCChar Npcindex, tHeading
                            'Leave sub
                            Exit Sub
...
'Tamed animal goes after player
Case 9
    If NPCList(Npcindex).Tamed <= 0 Then Exit Sub
    If NPCList(Npcindex).Target <= 0 Then Exit Sub
    If NPCList(Npcindex).Pos.map <> UserList(NPCList(Npcindex).Target).Pos.map Then Exit Sub
    tHeading = FindDirection(NPCList(Npcindex).Pos, UserList(MapData(NPCList(Npcindex).Pos.map, UserList(NPCList(Npcindex).Target).Pos.x, UserList(NPCList(Npcindex).Target).Pos.y).userindex).Pos)
    MoveNPCChar Npcindex, tHeading
       
'Tamed animal goes after NPC
Case 9
    If NPCList(Npcindex).Tamed <= 0 Then Exit Sub
    If NPCList(Npcindex).Target <= 0 Then Exit Sub
    If NPCList(Npcindex).Pos.map <> NPCList(NPCList(Npcindex).Target).Pos.map Then Exit Sub
    tHeading = FindDirection(NPCList(Npcindex).Pos, UserList(MapData(NPCList(Npcindex).Pos.map, NPCList(NPCList(Npcindex).Target).Pos.x, NPCList(NPCList(Npcindex).Target).Pos.y).Npcindex).Pos)
    MoveNPCChar Npcindex, tHeading
...
If obj.Pickable = 0 And UserList(userindex).Stats.Power < 5 Then
...
Call WriteVar(IniPath & "msgboard.txt", "POST" & NewNumPosts, "Author", UserList(userindex).Flags.StartName)
...
'IP ban
Dim NumBans2, NewNumBans2 As Integer
NumBans2 = Val(GetVar(IniPath & "ipban.txt", "INIT", "NumBANs"))
NewNumBans2 = NumBans2 + 1
Call WriteVar(IniPath & "ipban.txt", "INIT", "NumBANs", Str(NewNumBans2))
Call WriteVar(IniPath & "ipban.txt", "IPS", "IP" & NewNumBans2, UserList(userindex).IP)
...
Luck = UserList(userindex).Stats.Skill16
If Luck <= 40 Then Luck2 = 3
If Luck > 40 Then Luck2 = 2
DidHit = Int(RandomNumber(1, Luck2))
If Luck >= 80 Then DidHit = 1
...
UserList(userindex).UserTargetIndex = 0
    ...
    If NPCList(Npcindex).Movement = 4 Then
    UserList(userindex).Flags.Criminal = 2
...
Sub NPCAttackUser(Npcindex As Integer, userindex As Integer)
    ...
    '*****************************************************************
    Dim Hit As Integer, spot As Integer, SpotString As String, Luck, Luck2, Chance, DidHitx As Integer, y As Integer
    
    'Check to see if guard and player not criminal
    If NPCList(Npcindex).Guard = 1 And UserList(userindex).Flags.Criminal = 0 Then Exit Sub
    ...
    Luck = UserList(userindex).Stats.Skill6
    If Luck <= 50 Then Luck2 = 4
    If Luck > 50 Then Luck2 = 3
    If Luck > 80 Then Luck2 = 2
    Chance = Int(RandomNumber(1, Luck2))
    
    UserList(userindex).Npcindex = Npcindex
    
    
    If UserList(userindex).Flags.status <> 0 Then Exit Sub
    ...
    'Hit user
    If NPCList(Npcindex).CanAttack <> 1 Then Exit Sub
...
If UserList(victimindex).Flags.Criminal = 0 Then
'PK-free zone (CF and surroundings)
Select Case UserList(victimindex).Pos.map
Case 32
    Call SendData(ToIndex, attackerindex, 0, "@You are trying to attack someone in a PK-free area! You are now a criminal !" & FONTTYPE_INFO)
...
'PK protection
If Abs(UserList(attackerindex).Stats.ELV - UserList(victimindex).Stats.ELV) > 5 Then
    Call SendData(ToIndex, attackerindex, 0, "@This user is under level protection! You are now a criminal!" & FONTTYPE_INFO)
    UserList(attackerindex).Flags.Criminal = 2
    UserList(attackerindex).Flags.CriminalCount = UserList(attackerindex).Flags.CriminalCount + 45
    Exit Sub
End If
...
Luck = UserList(attackerindex).Stats.Skill16
If Luck <= 40 Then Luck2 = 3
If Luck > 40 Then Luck2 = 2
If Luck >= 80 Then Luck2 = 1

Luck = UserList(victimindex).Stats.Skill6
If Luck <= 40 Then Luck2 = Luck2 + 1
If Luck > 40 Then Luck2 = Luck2 + 2
If Luck >= 80 Then Luck2 = Luck2 + 3
...
If UserList(victimindex).Flags.Immortal = 1 Then
Call SendData(ToIndex, attackerindex, 0, "@Your attack is prevented by a mysterious force..." & FONTTYPE_INFO)
Exit Sub
End If

'Make sure attacker is over lvl 5 and not in duel
If UserList(attackerindex).Stats.ELV < 5 And UserList(attackerindex).Flags.Duel = 0 Then
Call SendData(ToIndex, attackerindex, 0, "@You are not experienced enough (You are a newbie) so you cannot attack other players yet. However, if both you and your victim types /DUEL you two will be able to fight eachother." & FONTTYPE_INFO)
Exit Sub
End If

'Newbie protection and not in duel
If UserList(victimindex).Stats.ELV < 5 And UserList(victimindex).Flags.Duel = 0 Then
Call SendData(ToIndex, attackerindex, 0, "@This player is under newbie protection and cannot be attacked until more experienced. However, if both you and your victim type /DUEL you two will be able to fight eachother." & FONTTYPE_INFO)
Exit Sub
End If
...
Call SendData(ToIndex, attackerindex, 0, "@You are attacking an innocent ! You are now a criminal !" & FONTTYPE_INFO)
...
Hit = Hit - (UserList(victimindex).Stats.DEF / 4)
If Hit < 1 Then Hit = 1
Else
...
UserList(victimindex).Npcindex = 0
UserList(victimindex).NPCtarget = 0
UserList(victimindex).UserTargetIndex = 0
    ...
    'Kill user
    Call SendData(ToPCArea, attackerindex, UserList(attackerindex).Pos.map, "@" & UserList(attackerindex).Name & " has slain " & UserList(victimindex).Name & " !" & FONTTYPE_INFO)
...
Call SendData(ToIndex, sendIndex, 0, "@Stats for: " & UserList(userindex).Name & " Level: " & UserList(userindex).Stats.ELV & ", Exp: " & UserList(userindex).Stats.EXP & "/" & UserList(userindex).Stats.ELU & FONTTYPE_INFO)
...
'Call SendData(ToMapButIndex, userindex, UserList(userindex).Pos.map, "MOC" & UserList(userindex).Char.CharIndex & "," & nPos.x & "," & nPos.y)
    Call SendData(ToPCAreaButIndex, userindex, UserList(userindex).Pos.map, "MOC" & UserList(userindex).Char.CharIndex & "," & nPos.x & "," & nPos.y)
...
'Call SendData(ToMap, 0, NPCList(Npcindex).Pos.map, "MOC" & NPCList(Npcindex).Char.CharIndex & "," & nPos.x & "," & nPos.y)
    Call SendData(ToNPCArea, Npcindex, NPCList(Npcindex).Pos.map, "MOC" & NPCList(Npcindex).Char.CharIndex & "," & nPos.x & "," & nPos.y)
...
Call ChangeNPCInv(userindex, Npcindex, NPCList(Npcindex).Object(40), 30)
...
Case 2 'Beggar
Call SendData(ToIndex, userindex, 0, "@The beggar says, Oh thank ye ! Thank ye ! I'm forever grateful !" & FONTTYPE_TALK)
UserList(userindex).Community.CommonRep = UserList(userindex).Community.CommonRep + 1
...
Call SendData(ToIndex, userindex, 0, "@Sorry, you do not have enough gold! Healing would cost you " & Charge & " gold!" & FONTTYPE_INFO)
Exit Sub
End If
...
Case 5 'Healer
UserList(userindex).Stats.MinHP = UserList(userindex).Stats.MaxHP
Call SendData(ToIndex, userindex, 0, "@You are now fully healed. That will be" & Charge & " gold." & FONTTYPE_INFO)
...
Call SendData(ToIndex, userindex, 0, "@The healer says ''You are ressurected. Welcome back. Be careful now and may the light bless you.''" & FONTTYPE_TALK)
UserList(userindex).Flags.status = 0
UserList(userindex).Char.Body = UserList(userindex).Char.LastBody
...
Else
Call SendData(ToIndex, userindex, 0, "@The healer grins and says ''Sure my friend, except your alive.''" & FONTTYPE_TALK)
End If

Case 61 'Priest Of Life
If UserList(userindex).Flags.status = 1 Then
Call SendData(ToIndex, userindex, 0, "@The Priest of Life says ''You are ressurected. Welcome back. Be careful now and may the light bless you.''" & FONTTYPE_TALK)
UserList(userindex).Flags.status = 0
UserList(userindex).Char.Body = UserList(userindex).Char.LastBody
UserList(userindex).Char.Head = UserList(userindex).Flags.StartHead
Call ChangeUserChar(ToMap, 0, UserList(userindex).Pos.map, userindex, UserList(userindex).Char.Body, UserList(userindex).Char.Head, UserList(userindex).Char.Heading, UserList(userindex).Char.WeaponAnim, UserList(userindex).Char.ShieldAnim)
...
Call SendData(ToIndex, userindex, 0, "DEN")
Else
Call SendData(ToIndex, userindex, 0, "@The Priest of Life grins and says ''Sure my friend, except your alive.''" & FONTTYPE_TALK)
End If
    ...
    Select Case UserList(userindex).NPCtarget
        ...
        Case 62 'Trainer
            If UserList(userindex).Stats.PracticePoints > 0 Then
                Call SendUserStatsBox(userindex)
                Call SendData(ToIndex, userindex, 0, "@The Trainer says ''Sure, I guess I can teach you a few tricks.''" & FONTTYPE_TALK)
                Call SendData(ToIndex, userindex, 0, "TRA")
            Else
                Call SendData(ToIndex, userindex, 0, "@The Trainer says ''Come back when you have some training points.''" & FONTTYPE_TALK)
        End If
...
'The damn thing causes a way to cheat
Call SendData(ToIndex, userindex, 0, "The gods will not grant you any miracles." & FONTTYPE_INFO)
Exit Sub
...
' Set limits
If UserList(userindex).Stats.MaxHP > 60 And UserList(userindex).Stats.EXP > 5000 And UserList(userindex).Stats.GLD > 5000 Then
...
'Mirace 1 (Give more strenght)
If Miracle = 1 Then
Call SendData(ToIndex, userindex, 0, "@The goddess of Hyliios has granted you one more strenght point !" & FONTTYPE_INFO)
UserList(userindex).Stats.MaxHP = UserList(userindex).Stats.MaxHP + 1
End If
...
'Mirace 1 (Give more strenght)
If Miracle = 1 Then
Call SendData(ToIndex, userindex, 0, "@The god Griigo has granted you one more strenght point !" & FONTTYPE_INFO)
UserList(userindex).Stats.MaxHP = UserList(userindex).Stats.MaxHP + 1
End If
...
'Mirace 1 (Give more strenght)
If Miracle = 1 Then
Call SendData(ToIndex, userindex, 0, "@The god Bendarr has granted you one more strenght point !" & FONTTYPE_INFO)
UserList(userindex).Stats.MaxHP = UserList(userindex).Stats.MaxHP + 1
End If
...
'Mirace 1 (Give more strenght)
If Miracle = 1 Then
Call SendData(ToIndex, userindex, 0, "@The godess Veega has granted you one more strenght point !" & FONTTYPE_INFO)
UserList(userindex).Stats.MaxHP = UserList(userindex).Stats.MaxHP + 1
End If
...
'Mirace 1 (Give more strenght)
If Miracle = 1 Then
Call SendData(ToIndex, userindex, 0, "@The god Zeendic has granted you one more strenght point !" & FONTTYPE_INFO)
UserList(userindex).Stats.MaxHP = UserList(userindex).Stats.MaxHP + 1
End If
...
Call SendData(ToIndex, userindex, 0, "@You throw the line into the water and wait... The blue bar represent how close the fish is to your rod." & FONTTYPE_INFO)
Exit Sub
End If
...
'PK-free zone (CF and surroundings)
Select Case UserList(UserTargetIndex).Pos.map
Case 32
    Call SendData(ToIndex, userindex, 0, "@You are pickpocket someone in a PK-free area! You are now a criminal !" & FONTTYPE_INFO)
    UserList(userindex).Flags.Criminal = 2
...
Call SendData(ToIndex, userindex, 0, "@This player is under newbie protection and cannot be pickpocked until more experienced." & FONTTYPE_INFO)
Exit Sub
End If
...
HaveFound = 1
If Int(RandomNumber(0, 0)) = 1 Then Exit For
End If
...
If Luck <= 10 Then Luck2 = 50
If Luck > 20 Then Luck2 = 45
If Luck > 30 Then Luck2 = 40
If Luck > 40 Then Luck2 = 35
If Luck > 50 Then Luck2 = 30
If Luck > 60 Then Luck2 = 25
If Luck > 70 Then Luck2 = 20
If Luck > 80 Then Luck2 = 15
If Luck > 90 Then Luck2 = 10
If Luck > 100 Then Luck2 = 3
...
UserList(UserTargetIndex).Object(PickObj2).ObjIndex = 0
OldAmount = UserList(UserTargetIndex).Object(PickObj2).Amount
UserList(UserTargetIndex).Object(PickObj2).Amount = 0
Call SendData(ToIndex, UserList(userindex).UserTargetIndex, 0, "PIC" & PickObj2)
...
Sub RoastMeat(userindex As Integer, slot As Byte)
    On Error Resume Next
    Dim Raise, obj As obj, x As Integer, y As Integer, map As Integer
...
End Sub
Sub CampHeal(userindex As Integer, slot As Byte)
    On Error Resume Next
    
    Dim obj As obj, x As Integer, y As Integer, map As Integer, Luck, Luck2, Chance
...
If UserList(UserTargetIndex).Flags.Criminal = 0 Then
If Spell.Destruction = 1 Then
'PK-free zone (CF and surroundings)
Select Case UserList(UserTargetIndex).Pos.map
Case 32
    Call SendData(ToIndex, userindex, 0, "@You are trying to attack someone in a PK-free area! You are now a criminal !" & FONTTYPE_INFO)
    UserList(userindex).Flags.Criminal = 2
    UserList(userindex).Flags.CriminalCount = UserList(userindex).Flags.CriminalCount + 45
    Exit Sub
Case 49
...
End Select
'PK protection
If Abs(UserList(userindex).Stats.ELV - UserList(UserTargetIndex).Stats.ELV) > 5 Then
    Call SendData(ToIndex, userindex, 0, "@This user is under level protection! You are now a criminal!" & FONTTYPE_INFO)
    UserList(userindex).Flags.Criminal = 2
    UserList(userindex).Flags.CriminalCount = UserList(userindex).Flags.CriminalCount + 45
    Exit Sub
End If
...
'Check range
If UserList(userindex).Pos.map <> UserList(UserList(userindex).UserTargetIndex).Pos.map Then
    Call SendData(ToIndex, userindex, 0, "@Your target is out of range..." & FONTTYPE_INFO)
    Exit Sub
End If

If Distance(UserList(userindex).Pos.x, UserList(userindex).Pos.y, UserList(UserList(userindex).UserTargetIndex).Pos.x, UserList(UserList(userindex).UserTargetIndex).Pos.y) > 6 Then
    Call SendData(ToIndex, userindex, 0, "@Your target is out of range..." & FONTTYPE_INFO)
    Exit Sub
End If

'Check to see if it isnt a gamemaster
If Spell.Destruction = 1 And UserList(UserList(userindex).UserTargetIndex).Flags.Immortal = 1 Then
Call SendData(ToIndex, userindex, 0, "@Your attack is prevented by a mysterious force..." & FONTTYPE_INFO)
Exit Sub
End If

'Make sure player is over lvl 5
If Spell.Destruction = 1 And UserList(userindex).Stats.ELV < 5 Then
SendData ToIndex, userindex, 0, "@This player is under newbie protection and cannot be attacked until more experienced." & FONTTYPE_FIGHT
Exit Sub
End If


'Make sure target is over lvl 5
If Spell.Destruction = 1 And UserList(UserTargetIndex).Stats.ELV < 5 Then
SendData ToIndex, userindex, 0, "@You cannot harm this player cause the player is not that experienced." & FONTTYPE_FIGHT
Exit Sub
End If

'Make sure attacker is over lvl 5
If Spell.Destruction = 1 And UserList(UserTargetIndex).Stats.ELV < 5 Then
SendData ToIndex, userindex, 0, "@You cannot harm other players because you are not experienced enough !" & FONTTYPE_FIGHT
Exit Sub
End If
...
'Check for spell casting abilities...
...
Case Else
'can't cast
Call SendData(ToIndex, userindex, 0, "@You are unable to cast any spells!" & FONTTYPE_INFO)
Exit Sub
End Select
...
UserList(UserTargetIndex).Stats.MinHP = UserList(UserTargetIndex).Stats.MinHP - Int(0.5 * Spell.DamageHp)
End If
...
UserList(UserTargetIndex).Char.Body = UserList(UserTargetIndex).Char.LastBody
...
'Check range
If UserList(userindex).Pos.map <> NPCList(UserTargetIndex).Pos.map Then
    Call SendData(ToIndex, userindex, 0, "@Your target is out of range..." & FONTTYPE_INFO)
    Exit Sub
End If

If Distance(UserList(userindex).Pos.x, UserList(userindex).Pos.y, NPCList(UserTargetIndex).Pos.x, NPCList(UserTargetIndex).Pos.y) > 6 Then
    Call SendData(ToIndex, userindex, 0, "@Your target is out of range..." & FONTTYPE_INFO)
    Exit Sub
End If
...
UserList(userindex).UserTargetIndex = 0
    ...
    'Give EXP maybe
    If UserList(userindex).Stats.ELV > NPCList(UserTargetIndex).Level Then
    Call SendData(ToIndex, userindex, 0, "@You do not get any experience ! You must be level " & NPCList(UserTargetIndex).Level & " or under to get experience from this creature !" & FONTTYPE_INFO)
    Else
    Call SendData(ToIndex, userindex, 0, "@You have gained " & NPCList(UserTargetIndex).GiveEXP & " experience !" & FONTTYPE_INFO)
...
NpcWillTake = Int(obj.Value * Luck2)
Else
...
Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''I'm afraid you can't afford this item.''" & FONTTYPE_TALK)
Exit Sub
End If
...
If sloto > MAX_INVENTORY_SLOTS Then
        ...
        Do Until UserList(userindex).Object(sloto).ObjIndex = 0
            ...
            If sloto > MAX_INVENTORY_SLOTS Then
                Call SendData(ToIndex, userindex, 0, "@You can't hold more items." & FONTTYPE_INFO)
                Exit Sub
                Exit Do
            End If
        Loop
...
Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''Here you go, take good care of it!" & FONTTYPE_TALK)
...
Sub NPCBuyItem(userindex As Integer, Npcindex As Integer, slot As Byte)
    ...
    If obj.Category = NPCList(Npcindex).Flags.Category1 Then WillBuy = 1
    If obj.Category = NPCList(Npcindex).Flags.Category2 Then WillBuy = 1
    If obj.Category = NPCList(Npcindex).Flags.Category3 Then WillBuy = 1
    If obj.Category = NPCList(Npcindex).Flags.Category4 Then WillBuy = 1
    If obj.Category = NPCList(Npcindex).Flags.Category5 Then WillBuy = 1
    
    If WillBuy = 0 Then
        Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''I am not interested in buying this item.''" & FONTTYPE_TALK)
        Exit Sub
    End If
    
    'WHOA... that's major bullshit
'    Randomize
'    Luck = UserList(userindex).Stats.Skill8
...
'    End If
    ...
    'Error trap
    If UserList(userindex).Object(slot).ObjIndex = 0 Then
    Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''Sell what?!''" & FONTTYPE_TALK)
    Exit Sub
    End If
    
    'Check to see if NPC has enough gold
    'If NPCList(Npcindex).Gold < NpcWillTake Then
    'Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''I'm afraid I can't afford this item.''" & FONTTYPE_INFO)
    'Exit Sub
    'End If
    
    'NPC DONT BUY IF NO VALUE IN IT
    If obj.Value = 0 Then
        Call SendData(ToIndex, userindex, 0, "@" & NPCList(Npcindex).Name & " says ''I see no value in this item, take it away.''" & FONTTYPE_TALK)
        Exit Sub
    End If
    
    'NPC dont buy if not sellable
    If obj.Sellable = 0 Then
        Call SendData(ToIndex, userindex, 0, "@You can't sell this item." & FONTTYPE_TALK)
        Exit Sub
    End If
...
Sub Meditate(userindex As Integer)
    ...
    Dim x As Integer, y As Integer, map As Integer, obj As obj
    ...
    If MapData(UserList(userindex).Pos.map, UserList(userindex).Pos.x, UserList(userindex).Pos.y).ObjInfo.ObjIndex = 289 Then
...
Luck = UserList(userindex).Stats.Skill27
If Luck < 10 Then Luck2 = 30
If Luck >= 10 Then Luck2 = 26
If Luck >= 30 Then Luck2 = 24
If Luck >= 40 Then Luck2 = 20
If Luck >= 50 Then Luck2 = 17
If Luck >= 60 Then Luck2 = 15
If Luck >= 70 Then Luck2 = 13
If Luck >= 80 Then Luck2 = 10
If Luck >= 90 Then Luck2 = 7
If Luck >= 100 Then Luck2 = 5
...
If UserList(userindex).Stats.MaxMAN < 50 And UserList(userindex).Stats.MinMAN < UserList(userindex).Stats.MaxMAN Then
...
UserList(userindex).Char.LastBody = UserList(userindex).Char.Body
...
If NPCList(Target).Tameable = 0 And UserList(userindex).Stats.Power < 8 Then
Call SendData(ToIndex, userindex, 0, "@This cannot be tamed." & FONTTYPE_INFO)
Exit Sub
End If

'Check to see if have enough skill ot tame
If UserList(userindex).Stats.Skill18 < NPCList(Target).SkillNeeded And UserList(userindex).Stats.Power < 8 Then
...
If UserList(userindex).Stats.Power < 8 Then
...
NPCList(Target).Attackable = 1

'Mark Owner name if not a god
If UserList(userindex).Stats.Power < 8 Then
NPCList(Target).Name = NPCList(Target).Name & " - " & UserList(userindex).Flags.StartName
End If

'Caused a strange bug...
'make it 50% weaker
'NPCList(Target).Stats.DEF = Int(NPCList(Target).Stats.DEF / 2)
'NPCList(Target).Stats.MaxHIT = Int(NPCList(Target).Stats.MaxHIT / 2)
'NPCList(Target).Stats.MaxHP = Int(NPCList(Target).Stats.MaxHP / 2)
'NPCList(Target).Stats.MinHIT = Int(NPCList(Target).Stats.MinHIT / 2)
'NPCList(Target).Stats.MinHP = Int(NPCList(Target).Stats.MinHP / 2)
'NPCList(Target).GiveEXP = Int(NPCList(Target).GiveEXP / 2)
'NPCList(Target).GiveGLD = Int(NPCList(Target).GiveGLD / 2)
...
UserList(userindex).Stats.EXP = UserList(userindex).Stats.EXP + 20
CheckUserLevel (userindex)
Call SendUserStatsBox(userindex)
...
If NPCList(AniTarget).Tameable <> 1 Then
Call SendData(ToIndex, userindex, 0, "@You cannot transfer ownership of animals if they are untamable." & FONTTYPE_INFO)
...
'Caused a strange bug...
'    'make it 50% stronger
'    NPCList(target4).Stats.DEF = Int(NPCList(target4).Stats.DEF * 2)
'    NPCList(target4).Stats.MaxHIT = Int(NPCList(target4).Stats.MaxHIT * 2)
'    NPCList(target4).Stats.MaxHP = Int(NPCList(target4).Stats.MaxHP * 2)
'    NPCList(target4).Stats.MinHIT = Int(NPCList(target4).Stats.MinHIT * 2)
'    NPCList(target4).Stats.MinHP = Int(NPCList(target4).Stats.MinHP * 2)
'    NPCList(target4).GiveEXP = Int(NPCList(target4).GiveEXP * 2)
'    NPCList(target4).GiveGLD = Int(NPCList(target4).GiveGLD * 2)
...
UserList(userindex).Stats.MaxHP = 45
UserList(userindex).Stats.MinHP = 45
...
GiveSpecial:
'give 15 exp to 3 special skills
temp1 = UserList(userindex).Flags.SpecSkill1
temp2 = UserList(userindex).Flags.SpecSkill2
temp3 = UserList(userindex).Flags.SpecSkill3
Select Case temp1
Case "Cooking"
UserList(userindex).Stats.Skill1 = UserList(userindex).Stats.Skill1 + 15
Case "Musicanship"
UserList(userindex).Stats.Skill2 = UserList(userindex).Stats.Skill2 + 15
Case "Tailoring"
UserList(userindex).Stats.Skill3 = UserList(userindex).Stats.Skill3 + 15
Case "Carpenting"
UserList(userindex).Stats.Skill4 = UserList(userindex).Stats.Skill4 + 15
Case "Lumberjacking"
UserList(userindex).Stats.Skill5 = UserList(userindex).Stats.Skill5 + 15
Case "Tactics"
UserList(userindex).Stats.Skill6 = UserList(userindex).Stats.Skill6 + 15
Case "Disguise"
UserList(userindex).Stats.Skill7 = UserList(userindex).Stats.Skill7 + 15
Case "Merchant"
UserList(userindex).Stats.Skill8 = UserList(userindex).Stats.Skill8 + 15
Case "Blacksmithing"
UserList(userindex).Stats.Skill9 = UserList(userindex).Stats.Skill9 + 15
Case "Hiding"
UserList(userindex).Stats.Skill10 = UserList(userindex).Stats.Skill10 + 15
Case "Magery"
UserList(userindex).Stats.Skill11 = UserList(userindex).Stats.Skill11 + 15
Case "Lockpicking"
UserList(userindex).Stats.Skill12 = UserList(userindex).Stats.Skill12 + 15
Case "Pickpocket"
UserList(userindex).Stats.Skill13 = UserList(userindex).Stats.Skill13 + 15
Case "Stealth"
UserList(userindex).Stats.Skill14 = UserList(userindex).Stats.Skill14 + 15
Case "Poisoning"
UserList(userindex).Stats.Skill15 = UserList(userindex).Stats.Skill15 + 15
Case "Swordmanship"
UserList(userindex).Stats.Skill16 = UserList(userindex).Stats.Skill16 + 15
Case "Parrying"
UserList(userindex).Stats.Skill17 = UserList(userindex).Stats.Skill17 + 15
Case "Animal Taming"
UserList(userindex).Stats.Skill18 = UserList(userindex).Stats.Skill18 + 15
Case "Religion Lore"
UserList(userindex).Stats.Skill19 = UserList(userindex).Stats.Skill19 + 15
Case "Fishing"
UserList(userindex).Stats.Skill20 = UserList(userindex).Stats.Skill20 + 15
Case "Mining"
UserList(userindex).Stats.Skill21 = UserList(userindex).Stats.Skill21 + 15
Case "Backstabbing"
UserList(userindex).Stats.Skill22 = UserList(userindex).Stats.Skill22 + 15
Case "Healing"
UserList(userindex).Stats.Skill23 = UserList(userindex).Stats.Skill23 + 15
Case "Surviving"
UserList(userindex).Stats.Skill24 = UserList(userindex).Stats.Skill24 + 15
Case "Etiquette"
UserList(userindex).Stats.Skill25 = UserList(userindex).Stats.Skill25 + 15
Case "Streetwise"
UserList(userindex).Stats.Skill26 = UserList(userindex).Stats.Skill26 + 15
Case "Streetwise"
UserList(userindex).Stats.Skill27 = UserList(userindex).Stats.Skill27 + 15
Case "Archery"
UserList(userindex).Stats.Skill28 = UserList(userindex).Stats.Skill28 + 15
End Select
...
Call SendData(ToIndex, userindex, 0, "@You assume the level of your target is " & UserList(USERTARGET).Stats.ELV & FONTTYPE_INFO)
...
'Animal attack/Settle subs:
Sub AnimalAttack(userindex As Integer)
Dim MyAnimal As Integer
Dim AttackTarget As Integer
Dim AttackTarget2 As Integer

MyAnimal = UserList(userindex).Stats.OwnAnimal
AttackTarget = UserList(userindex).NPCtarget
AttackTarget2 = UserList(userindex).UserTargetIndex
'Make sure NPC can be attacked
If NPCList(AttackTarget).Attackable = 0 Then
Call SendData(ToIndex, userindex, 0, "@This NPC cannot be attacked." & FONTTYPE_INFO)
Exit Sub
End If

'Look if it's a player
If AttackTarget <= 0 Then
NPCList(MyAnimal).Target = AttackTarget2
Call SendData(ToIndex, userindex, 0, "@Your pet attacks " & UserList(AttackTarget).Name & "!" & FONTTYPE_INFO)
Call SendData(ToIndex, AttackTarget2, 0, "@" & UserList(userindex).Name & "'s animal is attacking you!" & FONTTYPE_INFO)
NPCList(MyAnimal).Movement = 9
Else
NPCList(MyAnimal).Target = AttackTarget
Call SendData(ToIndex, userindex, 0, "@Your pet attacks your target !" & FONTTYPE_INFO)
NPCList(MyAnimal).Movement = 10
End If
NPCList(MyAnimal).Hostile = 1
NPCList(MyAnimal).CanAttack = 1



End Sub

Sub AnimalSettle(userindex As Integer)
Dim MyAnimal As Integer
Dim AttackTarget As Integer
MyAnimal = UserList(userindex).Stats.OwnAnimal
AttackTarget = NPCList(userindex).Target
NPCList(MyAnimal).Hostile = 0
NPCList(MyAnimal).CanAttack = 0
NPCList(MyAnimal).Movement = 6

'look if it's a player
If AttackTarget = UserList(userindex).UserTargetIndex Then
NPCList(MyAnimal).Target = 0
Call SendData(ToIndex, userindex, 0, "@Your pet attacks settles." & FONTTYPE_INFO)
Call SendData(ToIndex, AttackTarget, 0, "@" & UserList(userindex).Name & "'s animal stoped attacking you." & FONTTYPE_INFO)
Else
NPCList(MyAnimal).Target = 0
Call SendData(ToIndex, userindex, 0, "@Your pet settles..." & FONTTYPE_INFO)
End If
...
Sub UnstuckUser(userindex As Integer)
...
'Check if user might be jailed
If UserList(userindex).Pos.map = 142 Then
    Call SendData(ToIndex, userindex, 0, "@You cannot use the /unstuck command in CastleFall Castle." & FONTTYPE_INFO)
    Exit Sub
End If
...
If UserList(userindex).Flags.Meditate = 1 Then
    ...
    UserList(userindex).Flags.Meditate = 0
    Call SendData(ToIndex, userindex, 0, "ME2")
    Call SendData(ToIndex, userindex, 0, "@Upon your teleportation you snap out of your meditation trance." & FONTTYPE_INFO)
End If
...
UserList(userindex).Stats.MinHP = 1 'UserList(userindex).Stats.MaxHP
UserList(userindex).Stats.MinMAN = 1 'UserList(userindex).Stats.MaxMAN
UserList(userindex).Stats.MinSTA = 1 'UserList(userindex).Stats.MaxSTA

SendData ToIndex, userindex, 0, "@As you teleport, you lose your food and water." & FONTTYPE_FIGHT
'If UserList(userindex).Flags.Criminal = 2 Then
'SendData ToIndex, userindex, 0, "@You are no longer a criminal." & FONTTYPE_INFO
'UserList(userindex).Flags.Criminal = 0
'End If
...
UserList(userindex).Stats.MinHP = UserList(userindex).Stats.MaxHP
SendData ToIndex, userindex, 0, "@Alas, you have been teleported back to " & UserList(userindex).Town & "." & FONTTYPE_FIGHT

'loose gold if over level 5
If UserList(userindex).Stats.ELV > 5 And UserList(userindex).Stats.GLD > 299 Then
UserList(userindex).Stats.GLD = UserList(userindex).Stats.GLD - LoseGold
SendData ToIndex, userindex, 0, "@As you are teleported you loose " & LoseGold & " gold." & FONTTYPE_FIGHT
End If

'Loose 100 exp if over level 5 and have more than 99 exp
If UserList(userindex).Stats.ELV > 5 Then
UserList(userindex).Stats.EXP = UserList(userindex).Stats.EXP - LoseExp
SendData ToIndex, userindex, 0, "@The horrific experience of being teleported makes you loose " & LoseExp & " experience." & FONTTYPE_FIGHT
End If
...
Call CheckUserLevel(userindex)
'Call UpdateUserInv(True, userindex, Item)
If UserList(userindex).Race = "Human" Then Call WarpUserChar(userindex, CastleFallStartPos.map, CastleFallStartPos.x, CastleFallStartPos.y)
If UserList(userindex).Race = "Dark Elf" Then Call WarpUserChar(userindex, UgStartPos.map, UgStartPos.x, UgStartPos.y)
If UserList(userindex).Race = "Wood Elf" Then Call WarpUserChar(userindex, ValenStartPos.map, ValenStartPos.x, ValenStartPos.y)
If UserList(userindex).Race = "Haaki" Then Call WarpUserChar(userindex, DencStartPos.map, DencStartPos.x, DencStartPos.y)
