...
Public Const ToNPCArea = 8 'Send to all users in a npc's area
Public Const ToPCAreaButIndex = 9 'Send to all users in a user's area except user himself
...
UserList(userindex).Stats.MaxHIT = RandomNumber(5, 10)
UserList(userindex).Stats.MinHIT = UserList(userindex).Stats.MaxHIT - 2
UserList(userindex).Stats.DEF = RandomNumber(10, 15)

UserList(userindex).Stats.GLD = 200
UserList(userindex).Stats.BANKGLD = 100
UserList(userindex).Stats.Food = 0
UserList(userindex).Stats.PracticePoints = 5
...
On Error Resume Next
    ...
    Select Case sndRoute
        ...
        Case ToGM
            'Send to a GM
            For LoopC = 1 To LastUser
                If UserList(LoopC).Stats.Power > 0 Then frmMain.Socket2(LoopC).Write sndData, Len(sndData)
            Next LoopC
        Case ToAllButIndex
        ...
        Case ToPCAreaButIndex
            'Send to PC Area except user himse;f
            For y = UserList(sndIndex).Pos.y - MinYBorder - 5 To UserList(sndIndex).Pos.y + MinYBorder + 5
                For x = UserList(sndIndex).Pos.x - MinXBorder - 5 To UserList(sndIndex).Pos.x + MinXBorder + 5
        ...
        Case ToNPCArea
            'Send to NPC Area
            For y = NPCList(sndIndex).Pos.y - MinYBorder - 5 To NPCList(sndIndex).Pos.y + MinYBorder + 5
                For x = NPCList(sndIndex).Pos.x - MinXBorder - 5 To NPCList(sndIndex).Pos.x + MinXBorder + 5
                    If MapData(sndMap, x, y).userindex > 0 Then
                        frmMain.Socket2(MapData(sndMap, x, y).userindex).Write sndData, Len(sndData)
                    End If
                Next x
...
Call SendData(ToIndex, userindex, 0, "@GAME LOADING, PLEASE DON'T PRESS ANY KEYS" & FONTTYPE_INFO)
...
Call SendData(ToIndex, userindex, 0, "@Game loaded!" & FONTTYPE_INFO)
Call SendData(ToIndex, userindex, 0, "@Welcome to Era Online! Type /help for a list of important commands." & FONTTYPE_INFO)
Call SendData(ToIndex, userindex, 0, "@Report a bug or map error by typing /bug then a short description." & FONTTYPE_SKILLINFO)
TempVar1000 = GetVar(IniPath & "Server.ini", "INIT", "MOTD")
Call SendData(ToIndex, userindex, 0, "@" & TempVar1000)

'Clan sustem kind of defect, so just disable it...
...
If NPCList(UserList(userindex).Stats.AnimalIndex).Tameable <> 1 Then
Call SendData(ToIndex, userindex, 0, "@You cannot have ownership of animals if they are untamable." & FONTTYPE_INFO)
...
Call SendData(ToIndex, userindex, 0, "ERA")
...
If Left$(rdata, 5) = "ERASE" Then
    ...
    Else
    Call SendData(ToIndex, userindex, 0, "!!You do not have the newest version of EraOnline.exe and cannot play, please quit Era Online and get it at www.eraonline.net now under the download section.")
    CloseSocket (userindex)
    Exit Sub
    End If
...
If Left$(rdata, 6) = "NLOGIN" Then
    ...
    Else
    Call SendData(ToIndex, userindex, 0, "!!You do not have the newest version of EraOnline.exe and cannot play, please quit Era Online and get it at www.eraonline.net now.")
    CloseSocket (userindex)
    Exit Sub
    End If
...
'Player no longer criminal
If Left$(rdata, 3) = "CRM" Then
    rdata = Right$(rdata, Len(rdata) - 3)

UserList(userindex).Flags.Criminal = 0
UserList(userindex).Flags.CriminalCount = 0
Call SendData(ToIndex, userindex, 0, "@The memory of your criminal actions fail, and the people of Menath decide that you are fit to enter society again as a full citizen." & FONTTYPE_INFO)
...
'Check if moderated
'If MapData(UserList(userindex).Pos.map).Moderated = 1 Then
...
'Client cant see gfx ! Show it !
If LCase(rdata) = "/gfx" Then
    ...
    'Update...
    Call UpdateUserMap(userindex)
    Call SendData(ToIndex, userindex, 0, "@Better now? If not, make sure DirectX 7 or higher is installed. If you have and it still dosen't work, something could be wrong with the game, try re-installing." & FONTTYPE_INFO)
...
If Left$(rdata, 4) = "SAY;" Then
    ...
    If UserList(userindex).Flags.status = 0 Then
            ...
            Print #5, UserList(userindex).Name & ":" & rdata & " (" & Time & " " & Date & ")"
            Close #5
            'THIS "EXIT SUB" WAS MISSING!!! EE, NICE MOVE!!! THANK YOU FOR SCREWING UP MY TIME!!!
            Exit Sub
        Else
...
'ooc command
If LCase(Left$(rdata, 4)) = "/ooc" Then
    ...
    If UserList(userindex).Flags.status = 0 Then
            rdata = Right$(rdata, Len(rdata) - 5)
            Call SendData(ToPCArea, userindex, UserList(userindex).Pos.map, "@(OOC by " & UserList(userindex).Name & ": " & rdata & ")" & FONTTYPE_TALK)
            Open "Logs\" & "Zone" & UserList(userindex).Pos.map & ".log" For Append Shared As #5
            Print #5, UserList(userindex).Name & ":" & rdata & " (" & Time & " " & Date & ")"
            Close #5
            Exit Sub
...
If UserList(userindex).Flags.status = 0 Then
    ...
    Close #5
    Else
    Call SendData(ToPCArea, userindex, UserList(userindex).Pos.map, "@*" & UserList(userindex).Name & " seems to try to express something. But none can understand the ghostly movements." & FONTTYPE_TALK)
    Call SendData(ToPCArea, userindex, UserList(userindex).Pos.map, "PLW" & SOUND_HEART)
    Exit Sub
End If
Exit Sub
...
'Tell
If LCase(Left$(rdata, 5)) = "/msg " Then
    rdata = Right$(rdata, Len(rdata) - 5)
...
If UserList(userindex).Flags.status = 0 Then
    ...
    Exit Sub
    Else
    Call SendData(ToIndex, userindex, 0, "@You need to be alive to perform this action." & FONTTYPE_TALK)
    Exit Sub
End If
...
If UserList(userindex).Flags.status = 0 Then
    ...
    If UserList(userindex).Object(Val(rdata)).Equipped = 1 Then
    'Call SendData(ToIndex, userindex, 0, "@Please unequip the item first." & FONTTYPE_INFO)
    'Exit Sub
...
'Unstuck command
If LCase(rdata) = "/unstuck" Then
Call UnstuckUser(userindex)
Exit Sub
End If
...
If UserList(userindex).Flags.Battlemode = 1 Then
    ...
    Else
    Call SendData(ToIndex, userindex, 0, "@You need to switch to battle mode to perform this action." & FONTTYPE_TALK)
    Exit Sub
End If
End If
...
'STOP SKILL NOW
If UCase(Left(rdata, 3)) = "STP" Then
    rdata = Right$(rdata, Len(rdata) - 3)
...
If UCase(Left(rdata, 3)) = "PLY" Then
    rdata = Right$(rdata, Len(rdata) - 3)
...
'PERFORM SKILL NOW
If UCase(Left(rdata, 3)) = "PSN" Then
...
'Report bug
If LCase$(Left$(rdata, 5)) = "/bug " Then
    rdata = Right$(rdata, Len(rdata) - 5)
...
'Change Desc
If LCase$(Left$(rdata, 6)) = "/desc " Then
    If UserList(userindex).Flags.status = 0 Then
        rdata = Right$(rdata, Len(rdata) - 6)
        UserList(userindex).Desc = rdata
        Call SendData(ToIndex, userindex, 0, "@Description changed to ''" & rdata & "''." & FONTTYPE_INFO)
    Else
...
If UserList(userindex).Flags.status = 0 Then
     ...
     Else
     Call SendData(ToIndex, userindex, 0, "@You are dead and must be resurrected. No healer can heal your fatal wounds." & FONTTYPE_TALK)
     Exit Sub
End If
...
If rdata = "CMP" Then
If UserList(userindex).Flags.status = 0 Then
    Call CampHeal(userindex, Val(rdata))
    Else
    Call SendData(ToIndex, userindex, 0, "@You need to be alive to perform this action." & FONTTYPE_TALK)
Exit Sub
End If
...
'Ask a NPC to resurrect the player
...
Call WriteVar(IniPath & "signs.txt", "SIGN" & Sign, "Content", rdata & " - " & UserList(userindex).Flags.StartName)
Exit Sub
End If
...
rdata = Right$(rdata, Len(rdata) - 3)
If Abs(rdata) <> rdata Then Exit Sub
...
target3 = UserList(userindex).Stats.OwnAnimal
    ...
    NPCList(target3).Name = NPCList(target3).Name & " - " & UserList(userindex).Flags.StartName
    End If
...
Dim target321
target321 = UserList(userindex).Stats.OwnAnimal
'check to see if own animal at all
If target321 = 0 Then
Call SendData(ToIndex, userindex, 0, "@You dont own an animal !" & FONTTYPE_INFO)
Exit Sub
End If
Call AnimalSettle(userindex)
Exit Sub
End If
...
'Check if the user has any GM powers...
If UserList(userindex).Stats.Power <= 0 Then
Exit Sub
End If

'Open the GM tool
If UCase$(rdata) = "/GMTOOL" And UserList(userindex).Stats.Power > 0 Then
...
If Left$(rdata, 3) = "TGM" Then
    Dim GMxPos, GMyPos As Integer
    rdata = Right$(rdata, Len(rdata) - 3)
 
    For GMxPos = 47 To 58
        For GMyPos = 39 To 41
            If LegalPos(userindex, 212, GMxPos, GMyPos) Then
                Call WarpUserChar(userindex, 212, GMxPos, GMyPos)
                Exit Sub
            End If
        Next GMyPos
    Next GMxPos
    Call SendData(ToIndex, userindex, 0, "@No room in the Gm meeting place!" & FONTTYPE_INFO)
    Exit Sub
End If
...
'KICK AND BAN PLAYER
If Left$(rdata, 5) = "/BAN " Then
    rdata = Right$(rdata, Len(rdata) - 5)

    UserTargetIndex = NameIndex(rdata)
    If UserTargetIndex < 1 Then
        Call SendData(ToIndex, userindex, 0, "@User not online!" & FONTTYPE_INFO)
        Exit Sub
    End If
    Call KickBan(userindex, UserTargetIndex)
    Exit Sub
End If
...
If Left$(rdata, 3) = "RET" Then
    ...
    If UserList(tIndex).Flags.status = 1 Then
        ...
        UserList(tIndex).Char.Body = UserList(tIndex).Char.LastBody
        ...
        Call SendData(ToIndex, tIndex, 0, "@You have been resurrected by " & UserList(userindex).Name & " !" & FONTTYPE_INFO)
        Call SendData(ToPCArea, userindex, UserList(userindex).Pos.map, "PLW" & SOUND_CHORUS)
    Else
        Call SendData(ToIndex, tIndex, 0, "@Target is not dead and can therefor not be resurrected." & FONTTYPE_INFO)
    End If
    Exit Sub
End If
...
'(Un)set the criminal flag of a user...
If UCase$(Left$(rdata, 10)) = "/CRIMINAL " Then
    rdata = Right$(rdata, Len(rdata) - 10)
    ...
    End If
    If Val(Arg1) = 0 Then
        UserList(tIndex).Flags.Criminal = 0
        UserList(tIndex).Flags.CriminalCount = 0
        Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " made the memory, of you being a criminal, fade." & FONTTYPE_INFO)
        Call SendData(ToIndex, userindex, 0, "@You unmarked " & UserList(tIndex).Name & " as criminal." & FONTTYPE_INFO)
        SendUserStatsBox (tIndex)
    Else
        UserList(tIndex).Flags.Criminal = 2
        UserList(tIndex).Flags.CriminalCount = UserList(tIndex).Flags.CriminalCount + 45
        Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " marked you as criminal!" & FONTTYPE_FIGHT)
        Call SendData(ToIndex, userindex, 0, "@You marked " & UserList(tIndex).Name & " as criminal." & FONTTYPE_INFO)
        SendUserStatsBox (tIndex)
    End If
    Exit Sub
End If
...
'Jail an user
If UCase$(Left$(rdata, 6)) = "/JAIL " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    Dim JailX, JailY As Integer
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    If tIndex <= 0 Then
        Call SendData(ToIndex, userindex, 0, "@User not online." & FONTTYPE_INFO)
    ...
    For JailY = 13 To 18
        'Jail1
        For JailX = 73 To 75
            If LegalPos(tIndex, 142, JailX, JailY) Then
                Call WarpUserChar(tIndex, 142, JailX, JailY)
                Call SendData(ToIndex, userindex, 0, "@You have jailed " & UserList(tIndex).Name & " ." & FONTTYPE_INFO)
                Call SendData(ToIndex, tIndex, 0, "@You have been jailed by " & UserList(userindex).Name & " !" & FONTTYPE_FIGHT)
                Exit Sub
            End If
        Next JailX
        'Jail2
        ...
        Next JailX
    Next JailY
    Call SendData(ToIndex, userindex, 0, "@You failed to jail " & UserList(tIndex).Name & " ." & FONTTYPE_INFO)
    Exit Sub
End If
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 4 Then Exit Sub
'=====================================================================================
'=====================================================================================
...
'Take/Give some exp from/to an user
If UCase$(Left$(rdata, 5)) = "/EXP " Then
    ...
    If tIndex <= 0 Then
        ...
        Exit Sub
    End If
    UserList(tIndex).Stats.EXP = UserList(tIndex).Stats.EXP + Val(Arg1)
    Call CheckUserLevel(tIndex)
    Call SendUserStatsBox(tIndex)
    Exit Sub
End If

'Take/Give some gold from/to an user
If UCase$(Left$(rdata, 6)) = "/GOLD " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    If tIndex <= 0 Then
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 5 Then Exit Sub
'=====================================================================================
'=====================================================================================
...
'Tile wipe
If UCase(Left(rdata, 9)) = "/TILEWIPE" Then
    rdata = Right$(rdata, Len(rdata) - 9)
    Call TileWipe(userindex)
    Call SendData(ToIndex, userindex, 0, "@Performed a local area wipe." & FONTTYPE_INFO)
    Exit Sub
End If

'Map Junk WIPE
If UCase(Left(rdata, 8)) = "/MAPWIPE" Then
    rdata = Right$(rdata, Len(rdata) - 8)
    
    Dim Map2Clean As Integer
    Map2Clean = UserList(userindex).Pos.map
    Call SendData(ToAll, 0, 0, "@SERVER IS CLEANING MAP " & Map2Clean & " FOR JUNK...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT MINUTE." & FONTTYPE_INFO)
    Call SendData(ToAll, 0, 0, "@SERVER IS CLEANING MAP " & Map2Clean & " FOR JUNK...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT MINUTE." & FONTTYPE_INFO)
    Call SendData(ToAll, 0, 0, "@SERVER IS CLEANING MAP " & Map2Clean & " FOR JUNK...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT MINUTE." & FONTTYPE_INFO)
    'Save the users, just in case...
    Dim playeruser333 As Integer
    For playeruser333 = 1 To LastUser
        Call SaveUser(playeruser333, CharPath & UCase(UserList(playeruser333).Flags.StartName) & ".chr")
    Next playeruser333
    Call MapJunk(Map2Clean)
    Exit Sub
End If

'Change temp. the head of an user
If UCase$(Left$(rdata, 6)) = "/HEAD " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    If tIndex <= 0 Then
        ...
        Exit Sub
    End If
    UserList(tIndex).Char.Head = Val(Arg1)
    Call ChangeUserChar(ToMap, 0, UserList(tIndex).Pos.map, tIndex, UserList(tIndex).Char.Body, Val(Arg1), UserList(tIndex).Char.Heading, UserList(tIndex).Char.WeaponAnim, UserList(tIndex).Char.ShieldAnim)
    Exit Sub
End If

'Change the body of an user
If UCase$(Left$(rdata, 6)) = "/BODY " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    If tIndex <= 0 Then
        ...
        Exit Sub
    End If
    If UserList(userindex).Stats.Power < 8 And UserList(tIndex).Stats.Power < 1 Then
        Exit Sub
    End If
    UserList(tIndex).Char.Body = Val(Arg1)
    UserList(tIndex).Char.LastBody = Val(Arg1)
    Call ChangeUserChar(ToMap, 0, UserList(tIndex).Pos.map, tIndex, Val(Arg1), UserList(tIndex).Char.Head, UserList(tIndex).Char.Heading, UserList(tIndex).Char.WeaponAnim, UserList(tIndex).Char.ShieldAnim)
    Exit Sub
End If
...
'Rain/snow
'Weather crap makes the client sloooooooooooooowwwwwwwwwwww!!!!!!!!!!!
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 6 Then Exit Sub
'=====================================================================================
'=====================================================================================


'Alert
If UCase$(Left$(rdata, 7)) = "/ALERT " Then
...
'remove someones pet
If UCase$(Left$(rdata, 11)) = "/REMOVEPET " Then
    rdata = Right$(rdata, Len(rdata) - 11)
...
If UCase(Left(rdata, 9)) = "/JUNKWIPE" Then
    ...
    Call SendData(ToAll, 0, 0, "@SERVER IS CLEANING WORLD FOR JUNK...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT FEW MINUTES." & FONTTYPE_INFO)
    Call SendData(ToAll, 0, 0, "@SERVER IS CLEANING WORLD FOR JUNK...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT FEW MINUTES." & FONTTYPE_INFO)
    Dim playeruser3 As Integer
    For playeruser3 = 1 To LastUser
        Call SaveUser(playeruser3, CharPath & UCase(UserList(playeruser3).Flags.StartName) & ".chr")
    Next playeruser3
    Call WipeJunk
    Exit Sub
End If
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 7 Then Exit Sub
'=====================================================================================
'=====================================================================================
...
'Warp an user to a pos in the world
If UCase$(Left$(rdata, 6)) = "/WARP " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    ...
    If tIndex <= 0 Then
        ...
        Exit Sub
    End If
    If LegalPos(userindex, Val(Arg1), Val(Arg2), Val(Arg3)) Then
        Call WarpUserChar(tIndex, Val(Arg1), Val(Arg2), Val(Arg3))
    Else
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 8 Then Exit Sub
'=====================================================================================
'=====================================================================================

If UCase$(rdata) = "/SAVEWORLD" Then
    Dim PlayerUser90 As Integer
    Call SendData(ToAll, 0, 0, "@SERVER IS SAVING THE WORLD...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT FEW MINUTES." & FONTTYPE_INFO)
    Call SendData(ToAll, 0, 0, "@SERVER IS SAVING THE WORLD...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT FEW MINUTES." & FONTTYPE_INFO)
    Call SendData(ToAll, 0, 0, "@SERVER IS SAVING THE WORLD...PLEASE EXCUSE ANY POSSIBLE LAG FOR THE NEXT FEW MINUTES." & FONTTYPE_INFO)
    SaveWorld
    Exit Sub
...
'Change the head of an user
If UCase$(Left$(rdata, 9)) = "/NEWHEAD " Then
    rdata = Right$(rdata, Len(rdata) - 9)
    tIndex = NameIndex(ReadField(1, rdata, 32))
    Arg1 = ReadField(2, rdata, 32)
    If tIndex <= 0 Then
    ...
    End If
    UserList(tIndex).Char.Head = Val(Arg1)
    UserList(tIndex).Flags.StartHead = Val(Arg1)
    Call ChangeUserChar(ToMap, 0, UserList(tIndex).Pos.map, tIndex, UserList(tIndex).Char.Body, Val(Arg1), UserList(tIndex).Char.Heading, UserList(tIndex).Char.WeaponAnim, UserList(tIndex).Char.ShieldAnim)
    Exit Sub
End If
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 9 Then Exit Sub
'=====================================================================================
'=====================================================================================


'MOTD command
If UCase(Left(rdata, 6)) = "/MOTD " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    Call MOTD(rdata)
    Call SendData(ToIndex, userindex, 0, "@Changed the MOTD to:" & FONTTYPE_INFO)
    Call SendData(ToIndex, userindex, 0, "@" & rdata & FONTTYPE_FIGHT)
    Exit Sub
End If

'Give lock to someone
If UCase$(Left$(rdata, 10)) = "/GIVELOCK " Then
    rdata = Right$(rdata, Len(rdata) - 10)
    ...
    UserList(tIndex).Flags.Locks = UserList(userindex).Flags.Locks + 1
    Call SendData(ToIndex, tIndex, 0, "@You have been given one more lock." & FONTTYPE_INFO)
    Call SendData(ToIndex, userindex, 0, "@You have given one lock to " & rdata & "." & FONTTYPE_INFO)
    Exit Sub
End If
...
'Suspend a GM
If UCase$(Left$(rdata, 9)) = "/SUSPEND " Then
    rdata = Right$(rdata, Len(rdata) - 9)
    Dim NumWizs, Wiznum As Integer
    
    NumWizs = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs"))
    For Wiznum = 1 To NumWizs
        If UCase(rdata) = UCase(GetVar(IniPath & "Server.ini", "WIZLIST", "wiz" & Wiznum)) Then
            'Check if GM is already suspended...
            If Val(GetVar(IniPath & "Server.ini", "WIZLIST", "level & wiznum")) = 9 Then
                Call SendData(ToIndex, userindex, 0, "@That user is already suspended!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Check if GM has equal or higher powers...
            If Val(GetVar(IniPath & "Server.ini", "LEVEL" & GetVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum), "Power")) >= UserList(userindex).Stats.Power Then
                Call SendData(ToIndex, userindex, 0, "@That GM has equal or greater powers than you, thus cannot be suspended by you!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Suspend him
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "temp" & Wiznum, Str(GetVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum)))
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum, "9")
            tIndex = NameIndex(rdata)
            If tIndex > 0 Then
                Call ModName(tIndex)
                Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " suspended your GM powers!" & FONTTYPE_FIGHT)
            End If
            Call SendData(ToIndex, userindex, 0, "@You suspended " & rdata & " as GM!" & FONTTYPE_FIGHT)
            'log it
            Open App.Path & "\GMActions.log" For Append Shared As #5
                Print #5, "****" & (UserList(userindex).Name) & " suspended " & rdata & "as GM at" & Time & " " & Date
            Close #5
            Exit Sub
        End If
    Next Wiznum
    Call SendData(ToIndex, userindex, 0, "@There is no user with that name and GM powers" & FONTTYPE_INFO)
    Exit Sub
End If

'Unsuspend a GM
If UCase$(Left$(rdata, 11)) = "/UNSUSPEND " Then
    rdata = Right$(rdata, Len(rdata) - 11)
    Dim NumWizs2, Wiznum2 As Integer
    
    NumWizs2 = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs"))
    For Wiznum2 = 1 To NumWizs2
        If UCase(rdata) = UCase(GetVar(IniPath & "Server.ini", "WIZLIST", "wiz" & Wiznum2)) Then
            'Check if GM isn't suspended...
            If Val(GetVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum2)) <> 9 Then
                Call SendData(ToIndex, userindex, 0, "@That user isn't suspended!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Unsuspend him
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum2, Str(GetVar(IniPath & "Server.ini", "WIZLIST", "temp" & Wiznum2)))
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "temp" & Wiznum2, "9")
            tIndex = NameIndex(rdata)
            If tIndex > 0 Then
                Call ModName(tIndex)
                Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " unsuspended you!" & FONTTYPE_FIGHT)
            End If
            Call SendData(ToIndex, userindex, 0, "@You unsuspended " & rdata & "!" & FONTTYPE_FIGHT)
            'log it
            Open App.Path & "\GMActions.log" For Append Shared As #5
                Print #5, "****" & (UserList(userindex).Name) & " unsuspended " & rdata & " at" & Time & " " & Date
            Close #5
            Exit Sub
        End If
    Next Wiznum2
...
'=====================================================================================
'=====================================================================================
If UserList(userindex).Stats.Power < 10 Then Exit Sub
'=====================================================================================
'=====================================================================================
...
If UCase$(rdata) = "/SHUTDOWN" Then
    ...
    For playeruser2 = 1 To LastUser
        Call SaveUser(playeruser2, CharPath & UCase(UserList(playeruser2).Flags.StartName) & ".chr")
    Next playeruser2
    'Log it-
    Open App.Path & "\GMActions.log" For Append Shared As #5
...
'Change GM level
If UCase$(Left$(rdata, 9)) = "/CHANLEV " Then
    rdata = Right$(rdata, Len(rdata) - 9)
    Dim NumWizs3, Wiznum3 As Integer
    Arg1 = ReadField(1, rdata, 32)
    Arg2 = ReadField(2, rdata, 32)
    
    NumWizs3 = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs"))
    For Wiznum3 = 1 To NumWizs3
        If UCase(Arg1) = UCase(GetVar(IniPath & "Server.ini", "WIZLIST", "wiz" & Wiznum3)) Then
            'Check if GM isn't suspended...
            If Val(GetVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum)) = 9 Then
                Call SendData(ToIndex, userindex, 0, "@That user is suspended!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'check for illegal levels
            If Val(Arg2) = 9 Then
                Call SendData(ToIndex, userindex, 0, "@To unsuspend use /unsuspend 'GM' !" & FONTTYPE_INFO)
                Exit Sub
            End If
            If Val(Arg2) < 1 Then
                Call SendData(ToIndex, userindex, 0, "@Illegal level!" & FONTTYPE_INFO)
                Exit Sub
            End If
            If Val(Arg2) > Val(GetVar(IniPath & "Server.ini", "INIT", "NumLevels")) Then
                Call SendData(ToIndex, userindex, 0, "@Illegal level!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Ckeck if GM has equal or higher powers...
            If Val(GetVar(IniPath & "Server.ini", "LEVEL" & GetVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum3), "Power")) >= UserList(userindex).Stats.Power Then
                Call SendData(ToIndex, userindex, 0, "@That GM has equal or greater powers than you, thus you cannot change his level!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Check if level is equal or higher
            If Val(GetVar(IniPath & "Server.ini", "LEVEL" & Arg2, "Power")) >= UserList(userindex).Stats.Power Then
                Call SendData(ToIndex, userindex, 0, "@You cannot grand someone the same or higher level than you!" & FONTTYPE_INFO)
                Exit Sub
            End If
            'Change him
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "level" & Wiznum3, Arg2)
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "temp" & Wiznum3, Arg2)
            tIndex = NameIndex(Arg1)
            If tIndex > 0 Then
                Call ModName(tIndex)
                Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " changed your GM level to " & GetVar(IniPath & "Server.ini", "LEVEL" & Val(Arg2), "Power") & "!" & FONTTYPE_FIGHT)
            End If
            Call SendData(ToIndex, userindex, 0, "@You changed " & Arg1 & " his GM level to " & GetVar(IniPath & "Server.ini", "LEVEL" & Val(Arg2), "Power") & "!" & FONTTYPE_FIGHT)
            'log it
            Open App.Path & "\GMActions.log" For Append Shared As #5
                Print #5, "****" & (UserList(userindex).Name) & " ChangLev-ed " & Arg1 & " to " & GetVar(IniPath & "Server.ini", "LEVEL" & Val(Arg2), "Power") & " at " & Time & " " & Date
            Close #5
            Exit Sub
        End If
    Next Wiznum3
...
'Add a GM
If UCase$(Left$(rdata, 7)) = "/ADDGM " Then
    rdata = Right$(rdata, Len(rdata) - 7)
    Dim Temp12345, WizNum5, NumWizs5 As Integer
    Arg1 = rdata 'ReadField(1, rdata, 32)
    
    If FileExist(CharPath & UCase(Arg1) & ".chr", vbNormal) = False Then
        Call SendData(ToIndex, userindex, 0, "@User does not exist.")
        Exit Sub
    End If
    
    NumWizs5 = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs"))
    For WizNum5 = 1 To NumWizs5
        If UCase(Arg1) = UCase(GetVar(IniPath & "Server.ini", "WIZLIST", "wiz" & WizNum5)) Then
            Call SendData(ToIndex, userindex, 0, "@User is already a GM.")
            Exit Sub
        End If
    Next WizNum5
    
    Temp12345 = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs")) + 1
    Call WriteVar(IniPath & "Server.ini", "INIT", "NumWizs", Str(Temp12345))
    Call WriteVar(IniPath & "Server.ini", "WIZLIST", "wiz" & Temp12345, Arg1)
    Call WriteVar(IniPath & "Server.ini", "WIZLIST", "level" & Temp12345, "0")
    Call WriteVar(IniPath & "Server.ini", "WIZLIST", "temp" & Temp12345, "0")
    Call SendData(ToIndex, userindex, 0, "@You added " & Arg1 & " to the GameMaster list." & FONTTYPE_INFO)
    tIndex = NameIndex(Arg1)
    If tIndex > 0 Then
        Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " added you to the GameMaster list." & FONTTYPE_INFO)
    End If
    'log it
    Open App.Path & "\GMActions.log" For Append Shared As #5
        Print #5, "****" & (UserList(userindex).Name) & " added GM " & Arg1 & " at " & Time & " " & Date
    Close #5
    Exit Sub
End If

'Remove a GM
If UCase$(Left$(rdata, 10)) = "/REMOVEGM " Then
    rdata = Right$(rdata, Len(rdata) - 10)
    Dim NumWizs4, WizNum4 As Integer
    Arg1 = ReadField(1, rdata, 32)

    NumWizs4 = Val(GetVar(IniPath & "Server.ini", "INIT", "NumWizs"))
    For WizNum4 = 1 To NumWizs4
        If UCase(Arg1) = UCase(GetVar(IniPath & "Server.ini", "WIZLIST", "wiz" & WizNum4)) Then
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "wiz" & WizNum4, " ")
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "level" & WizNum4, "0")
            Call WriteVar(IniPath & "Server.ini", "WIZLIST", "temp" & WizNum4, "0")
            Call SendData(ToIndex, userindex, 0, "@You removed " & Arg1 & " from the GameMaster list." & FONTTYPE_INFO)
            tIndex = NameIndex(Arg1)
            If tIndex > 0 Then
                Call SendData(ToIndex, tIndex, 0, "@" & UserList(userindex).Name & " removed you from the GameMaster list." & FONTTYPE_INFO)
                Call ModName(tIndex)
            End If
            'log it
            Open App.Path & "\GMActions.log" For Append Shared As #5
                Print #5, "****" & (UserList(userindex).Name) & " removed GM " & Arg1 & " at " & Time & " " & Date
            Close #5
            Exit Sub
        End If
    Next WizNum4
    Call SendData(ToIndex, userindex, 0, "@" & Arg1 & " was not found in the GameMaster list." & FONTTYPE_INFO)
    Exit Sub
End If
